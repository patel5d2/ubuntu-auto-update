...
func TestHandleReport_Success(t *testing.T) {
	...
	// New nested wire format: the agent sends update_results.apt_output, which
	// the backend maps to update_output. system_info carries os/kernel.
	body, _ := json.Marshal(map[string]interface{}{
		"hostname":      "test-host",
		"agent_version": "1.2.3",
		"update_results": map[string]interface{}{
			"apt_output":         "update",
...
func TestHandleReport_DBError(t *testing.T) {
	...
	mock.ExpectQuery(`INSERT INTO hosts`).
		WithArgs("test-host", "root", "update", "", sql.NullString{}, false, 0, 0, "", "", "").
		WillReturnError(sql.ErrConnDone)
